...
- Sau khi xong, ghi tóm tắt vào `docs/reports/PA-OVERHAUL-REPORT.md`: danh sách file đã sửa + root cause từng nhóm lỗi + ghi chú remaining issues. KHÔNG kết luận PASS nếu Policy Detail còn field Motor hoặc HĐ mới không mở được từ danh sách.
